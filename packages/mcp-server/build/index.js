...
`,m=he;var l=new be({name:"rushdb-mcp-server",version:"1.0.0"},{capabilities:{tools:{list:!0,call:!0},prompts:{}}});l.setRequestHandler(we,async()=>({tools:k}));l.setRequestHandler(Be,async()=>({prompts:[{name:"rushdb.queryBuilder",description:"RushDB Query Builder system prompt: guides the model to discover labels/properties first and construct validated SearchQuery objects before calling find-related tools.",arguments:[]}]}));l.setRequestHandler(ve,async r=>{let t=r.params.name;if(t!=="rushdb.queryBuilder")throw new se(oe.InvalidRequest,`Unknown prompt: ${t}`);return{description:"RushDB Query Builder system prompt to enable discovery-first, schema-safe SearchQuery construction before find-related tool calls.",messages:[{role:"user",content:{type:"text",text:m}}]}});l.setRequestHandler(Ie,async r=>{let t=r.params.name,e=r.params.arguments||{};try{switch(t){case"FindLabels":let n=await P({where:e.where,limit:e.limit,skip:e.skip,orderBy:e.orderBy});return{content:[{type:"text",text:n.length>0?n.map(D=>`${D.name}: ${D.count} records`).join(`
