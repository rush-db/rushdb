export type TGetFirstArgument<T> = T extends (first: infer FirstArgument, ...args: any[]) => any
  ? FirstArgument
  : never
...
export type TReplace<T, K extends keyof T, R> = TIsUnion<K> extends false
  ? TFlattenTypes<Omit<T, K> & { [key in T as K]: R }>
  : never
