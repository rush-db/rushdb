...
export class QueryController {
  ...
  @HttpCode(HttpStatus.OK)
  async runCypher(
    @ExternalTransactionDecorator() transaction: Transaction,
    @Body() body: CypherDto,
    ...
    // NOTE: Raw Cypher is only available for external-DB projects (customDb is set).
    // KU compute_operation is intentionally NOT emitted here — compute-heavy KU tracking
    // applies to RushDB-managed (shared) instances only.
