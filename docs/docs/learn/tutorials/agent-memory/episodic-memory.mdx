---
slug: /tutorials/episodic-memory
sidebar_position: 23
...
console.log('Session goal ID:', goal.__id)
```
...
const step1 = await recordStep(goal.__id, 'tool_call', 'Search documentation: graph traversal patterns', 1)
```
...
const context = await resumeSession(goal.__id)
...
```typescript
await db.records.update(goal.__id, {
  status: 'completed',
...
| Team memory ([Building Team Memory](/tutorials/building-team-memory)) | Shared persistent knowledge: tickets, docs, decisions |
| Fact memory ([RushDB as a Memory Layer](/tutorials/memory-layer))     | Long-lived facts about entities across all sessions   |
...
- [RushDB as a Memory Layer](/tutorials/memory-layer) — long-lived facts and entity profiles
- [Building Team Memory](/tutorials/building-team-memory) — shared knowledge graph for agents
- [Agent-Safe Query Planning](/tutorials/agent-safe-query-planning) — grounded query execution guard
