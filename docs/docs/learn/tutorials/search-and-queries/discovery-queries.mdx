---
slug: /tutorials/discovery-queries
sidebar_position: 20
...
- [Agent-Safe Query Planning with Ontology First](/tutorials/agent-safe-query-planning) — automate the discovery loop in an agent
- [MCP Quickstart for Real Operators](/deploy/mcp-operator-quickstart) — the same workflow via the MCP server
- [Thinking in Graphs](/tutorials/thinking-in-graphs) — mental model for working with connected data
