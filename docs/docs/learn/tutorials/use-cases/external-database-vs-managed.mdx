---
sidebar_position: 38
title: 'External Database vs Managed vs Self-Hosted'
description: Compare RushDB's Managed, External Database, and Self-Hosted operating models with a feature matrix, decision guide, and migration path.
tags: [Deployment, External Database, Neo4j, Self-Hosted]
---

import Tabs from '@site/src/components/LanguageTabs'
import TabItem from '@theme/TabItem'

# External Database vs Managed vs Self-Hosted
...
|                               | **Managed**           | **External Database**        | **Self-Hosted**           |
| ----------------------------- | --------------------- | ---------------------------- | ------------------------- |
| **Who runs RushDB**           | RushDB Cloud          | RushDB Cloud                 | You                       |
| **Your Neo4j instance**       | RushDB-managed        | Yours (Aura or local)        | Yours                     |
| **Data residency**            | RushDB infrastructure | Your Neo4j/Aura location     | Your infrastructure       |
| **Setup time**                | < 1 minute            | ~15 minutes                  | 30–60 minutes             |
| **Ops burden**                | None                  | Minimal                      | Full                      |
| **Raw Cypher access**         | ✗                     | ✓                            | ✓                         |
| **Custom embedding endpoint** | ✗                     | ✗                            | ✓                         |
| **Managed billing**           | RushDB plans          | RushDB plan + database costs | Infrastructure costs only |
...
### External Database — connect your Neo4j

**Choose External Database when:**
...
An External Database project connects RushDB Cloud to credentials for a Neo4j or Aura database you operate separately. RushDB's API remains hosted by RushDB; this is not a customer-cloud RushDB deployment. See [Connect an Aura Instance](/deploy/guides/connect-aura) for the full setup guide.

<Tabs groupId="programming-language">
<TabItem value="typescript" label="TypeScript">

```typescript
// Same SDK interface — the External Database is transparent to application code
import RushDB from '@rushdb/javascript-sdk'
const db = new RushDB('rbk_your_external_db_project_key')
```
...
db = RushDB("rbk_your_external_db_project_key")
```
...
curl -X POST https://api.rushdb.com/api/v1/query/raw \
  -H "Authorization: Bearer rbk_your_external_db_project_key" \
  -H "Content-Type: application/json" \
...
See the [Deployment guide](/deploy/remote-hosting/self-hosting-rushdb) for the complete Docker Compose walkthrough.
...
| Feature                               | Managed | External Database | Self-Hosted |
| ------------------------------------- | ------- | ----------------- | ----------- |
...
Managed and External Database projects share the same RushDB API layer. There is no direct data export between Neo4j instances because the target uses a Neo4j database that you operate separately. The migration flow is:

**1. Document your data shape**

Use `getSchemaMarkdown()` to capture your record labels, property names, and relationship types before migrating:

<Tabs groupId="programming-language">
<TabItem value="typescript" label="TypeScript">

```typescript
const schema = await db.getSchemaMarkdown()
...
**3. Create an External Database project**

In the [RushDB Dashboard](https://app.rushdb.com), create a new project using **Use my own Neo4j instance** and follow the [External Aura setup guide](/deploy/guides/connect-aura).

**4. Re-import into the External Database project**
...
```typescript
const externalDb = new RushDB('rbk_external_db_project_key')

await externalDb.records.importJson({
  label: 'Product',
  data: payload
})
```
...
```python
external_db = RushDB('rbk_external_db_project_key')

external_db.records.import_json(
    label='Product',
    data=payload
)
```
...
```bash
EXTERNAL_DB_TOKEN="rbk_external_db_project_key"

curl -s -X POST "$BASE/records/import" \
  -H "$H" -H "Authorization: Bearer $EXTERNAL_DB_TOKEN" \
...
### External Database / Managed → Self-Hosted

Self-hosted uses the same REST API surface as the cloud. The migration steps are:

1. [Deploy your self-hosted instance](/deploy/remote-hosting/self-hosting-rushdb) with `RUSHDB_SELF_HOSTED=true`.
...
```typescript
const selfHostedDb = new RushDB(process.env.SELFHOSTED_API_KEY, {
...
- **Get started with Managed** — [Quickstart](/get-started/quick-tutorial)
- **Connect an External Database** — [Connect an Aura Instance](/deploy/guides/connect-aura)
- **Deploy self-hosted** — [Deployment](/deploy/remote-hosting/self-hosting-rushdb)
- **Manage projects post-deploy** — [Self-Hosted Project Setup](/deploy/guides/self-hosted-project-setup)
