...
Flat rows only — no nested objects. For nested data use [`import_json`](/build/data/import-data).
...
Flat rows only — no nested objects. For nested data use [`importJson`](/build/data/import-data).
...
- [Import Data](/build/data/import-data) — import nested JSON or CSV (auto-creates relationships)
- [Find & Query](/build/data/find-and-query) — search and filter records
- [Connect Records](/build/graph/connect-records) — attach and detach relationships
- [Transactions](/build/reliability/transactions) — ACID guarantees for multi-step writes
- [Write Records with Vectors](/build/ai-search/write-with-vectors) — attach embedding vectors
