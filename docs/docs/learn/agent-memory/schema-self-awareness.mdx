...
The ontology comes in two formats: a compact Markdown document built for LLM context injection, and structured JSON for programmatic use.

<ThemeTabs groupId="ontology-format">
  <TabItem value="markdown" label="Markdown" default>

`getOntologyMarkdown()` serialises the graph into a token-efficient document ready to paste into a system prompt:
...
`getOntology()` returns the same schema as an `OntologyItem[]` array — one item per label:
...
- [Discover Your Schema](/learn/records-and-queries/discover-your-schema) — full ontology API reference
