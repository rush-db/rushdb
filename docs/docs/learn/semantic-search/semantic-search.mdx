...
If you want RushDB to convert a natural-language request into a `SearchQuery`, use [Smart Search](/learn/semantic-search/smart-search) with `db.ai.search(prompt)`.
...
- [Smart Search](/learn/semantic-search/smart-search) — generate and execute SearchQuery from natural language
