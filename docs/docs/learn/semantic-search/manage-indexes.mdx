---
slug: /build/ai-search/manage-indexes
sidebar_position: 2
...
An **embedding index** is a policy that tells RushDB to vectorize a specific string property for a label. Once `status` is `ready`, every record matching that label+property pair is searchable via [`db.ai.search()`](/build/ai-search/semantic-search).
...
| `sourceType`         | `string` | no       | `"managed"` (default) or `"external"`. See [Bring Your Own Vectors](/build/ai-search/bring-your-own-vectors). |
...
- [Semantic Search](/build/ai-search/semantic-search) — search using managed or external indexes
- [Write Records with Vectors](/build/ai-search/write-with-vectors) — attach vectors at write time
- [Bring Your Own Vectors](/build/ai-search/bring-your-own-vectors) — external embedding workflow
