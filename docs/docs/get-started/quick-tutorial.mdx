...
## Step 5: Prefiltered Semantic Search

Combine a `where` filter with `db.ai.search` to scope semantic search to a subset of records. RushDB automatically uses an **ENN (exact nearest-neighbor) prefilter** — it matches records by field value first, then ranks by similarity.
...
:::note ANN vs ENN
Without a `where` clause, RushDB runs **ANN** (approximate nearest-neighbor) directly on the vector index — very fast. Adding `where` switches to **ENN prefilter**: RushDB matches records by field first, then scores the filtered set by similarity. ENN is exact but scans only the matched subset, so it stays fast at scale.
:::
...
- Go deeper on AI features: [AI & Semantic Search](../typescript-sdk/ai)
- Check out end-to-end tutorials in the [Tutorials](../tutorials/reusable-search-query) section
