...
Sessions are JWT-signed using `RUSHDB_AES_256_ENCRYPTION_KEY` (HS256 by default; RS256 when `RUSHDB_JWT_PRIVATE_KEY` is configured). See [Environment Variables — MCP OAuth](/deploy/config-env-variables#mcp-oauth--rs256-jwt-signing) for the RS256 key setup.
...
All API calls require a valid API key passed as the `Authorization: Bearer <token>` header. See [Authorization](/deploy/get-api-key) for key management.
