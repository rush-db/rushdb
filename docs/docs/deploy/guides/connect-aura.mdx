---
sidebar_position: 34
title: 'Connecting an External Neo4j Aura Database'
description: Connect your own Neo4j Aura database to RushDB Cloud while continuing to use the full RushDB API.
tags: [Deployment, External Database, Neo4j, Aura]
---

import Tabs from '@site/src/components/LanguageTabs'
import TabItem from '@theme/TabItem'

# Connecting an External Neo4j Aura Database

An **External Database** project uses RushDB's hosted API and query layer with a Neo4j or Aura instance that you operate separately. Graph records, relationships, and vectors are stored in that database; the RushDB runtime still runs on RushDB infrastructure.

This is not a BYOC deployment. The term **BYOC** is reserved for the future model where the RushDB runtime itself runs in the customer's cloud account.
...
:::tip Aura tier compatibility
Aura capabilities vary by tier and version. Choose a tier that exposes all [required APOC procedures](/deploy/infrastructure/neo4j-and-aura#apoc-core-procedures-used) and passes RushDB's connection verification.
:::

---

## Step 2: Create an External Database project in the RushDB dashboard
...
External Database and self-hosted projects have access to `POST /api/v1/query/raw`. Use it to confirm data is landing in Aura directly.
...
## External Aura with a self-hosted RushDB instance
...
See [Self-Hosting RushDB](/deploy/remote-hosting/self-hosting-rushdb) for the complete Docker Compose setup.

---

## What changes with an External Database vs managed

|                   | Managed (default)    | External Database                     |
| ----------------- | -------------------- | ------------------------------------- |
| Data location     | RushDB-managed Neo4j | Your separately operated Aura / Neo4j |
| Raw Cypher access | No                   | Yes — `POST /query/raw`               |
| Billing           | KU-based             | KU-based; database costs are separate |
| Wipe / restore    | Via RushDB dashboard | Use your Neo4j/Aura backup tools      |
| SLA for the graph | RushDB SLA           | Your Neo4j/Aura service terms         |

See [External Database vs Managed vs Self-Hosted](/learn/tutorials/use-cases/external-database-vs-managed) for a full comparison.

---

## Next steps

- [External Database vs Managed vs Self-Hosted](/learn/tutorials/use-cases/external-database-vs-managed) — choose the right topology
- [Self-Hosting RushDB](/deploy/remote-hosting/self-hosting-rushdb) — run both RushDB and the graph on your own infra
- [Project Setup After Deployment](/deploy/guides/self-hosted-project-setup) — configure embedding models and team access per project
