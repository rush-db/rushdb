---
title: Neo4j & Aura
description: Configure RushDB's graph backend — bundled Neo4j via Docker Compose, an existing Neo4j instance, or Neo4j Aura as an External Database.
sidebar_position: 1
displayed_sidebar: docs
tags: [Infrastructure, Neo4j, Aura, External Database, Self-Hosted]
---
...
| **External Neo4j Aura**      | Use a customer-operated Aura database with the RushDB API layer |
...
## Option C — External Neo4j Aura Database

Connect a Neo4j Aura instance that you operate separately while RushDB provides the API layer. In RushDB Cloud, the RushDB runtime still runs on RushDB infrastructure, so this connection mode is called **External Database**, not BYOC.

**Why use an External Database?**

- Graph data is stored in your Neo4j Aura tenancy
- Keep control of the Aura subscription, region, credentials, backups, and capacity
- Use Neo4j's managed backups, monitoring, and scaling

:::info APOC on Aura
APOC availability varies by Aura tier and Neo4j version. RushDB requires the procedures listed above, so use an Aura configuration that passes **Verify Connection**. Do not choose a tier based only on its name.
:::
...
4. When creating a project in the RushDB dashboard, enable **Use my own Neo4j instance** and enter the same credentials

:::info
External Database projects unlock raw Cypher access via `POST /api/v1/db/{projectId}/query/raw` — useful for advanced graph traversals outside the standard RushDB query model.
:::
