...
def search_articles(params: dict) -> dict:
    ...
    return {
        "items": [{"id": a.id, "title": a.data.get("title"), "category": a.data.get("category")} for a in result.data],
        "total": result.total
    }
```
...
def semantic_article_search(params: dict) -> list[dict]:
    ...
    return [
        {"id": a.id, "title": a.data.get("title"), "score": a.data.get("__score"), "category": a.data.get("category")}
...
def enriched_search(user_query: str) -> list[dict]:
    ...
    def enrich(article):
        ...
        return {
            ...
            "score": article.data.get("__score"),
            "author": author_result.data[0].data.get("name") if author_result.data else None
        }
...
def paginated_articles(category: str, page: int, page_size: int = 20) -> dict:
    ...
    return {
        "items": [{"id": a.id, "title": a.data.get("title")} for a in result.data],
        "total": result.total,
...
- [Hybrid Retrieval](./hybrid-retrieval.mdx) — two-phase structured-filter + semantic-rank for AI pipelines
- [Explainable Results](./explainable-results.mdx) — surface evidence for every search result
- [Query Optimization](./query-optimization.mdx) — shape queries for throughput and cost efficiency
