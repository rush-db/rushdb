...
def test_sdk_returns_in_stock_widgets():
    ...
    assert result.data[0].data["sku"] == "P-001"
...
## Step 3: Test aggregation parity

Aggregate queries are the most likely to diverge between SDK versions. Test them explicitly.

<Tabs groupId="programming-language">
<TabItem value="typescript" label="TypeScript">

```typescript
it('aggregate count by category returns correct totals', async () => {
...
def test_aggregate_count_by_category():
    ...
    assert result.data[0].data["category"] == "widgets"
    assert result.data[0].data["count"] == 2
```
