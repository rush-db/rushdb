...
- [Audit Trails with Immutable Events](./audit-trails.mdx) — separate event log from current state for reconstructible history
- [Versioning Records Without Losing Queryability](./record-versioning.mdx) — keeping historical state queryable alongside current state
- [RushDB as a Memory Layer](./memory-layer.mdx) — using the same EPISODE + REFERENCE pattern for agent memory
