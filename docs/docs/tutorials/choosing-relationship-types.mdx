...
```typescript
const order = await db.records.create({ label: 'ORDER', data: { total: 149.00 } })
...
const userStats = await db.records.find({
  ...
  aggregate: {
    ...
    documentsAuthored: { fn: 'count', alias: '$authored', unique: true }
  },
...
pairs = db.records.find({
    ...
    "aggregate": {"documentId": "$record.__id", "userId": "$user.__id"}
})
...
PAIRS=$(curl -s -X POST "$BASE/records/search" \
  -H "$H" -H "Authorization: Bearer $TOKEN" \
  -d '{"labels":["DOCUMENT"],"where":{"USER":{"$alias":"$user"}},"aggregate":{"documentId":"$record.__id","userId":"$user.__id"}}')
...
- [Modeling Hierarchies, Networks, and Feedback Loops](./modeling-hierarchies.mdx) — three common graph shapes and how to query each
- [Thinking in Graphs: From Tables to Traversals](./thinking-in-graphs.mdx) — the full mental model shift
- [SearchQuery Deep Dive](./searchquery-advanced-patterns.mdx) — `$relation`, `$alias`, and aggregation patterns
