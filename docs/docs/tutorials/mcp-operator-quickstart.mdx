...
The [MCP quickstart](../mcp-server/quickstart.mdx) covers installation in three lines. This tutorial covers what happens after the config file is saved.
...
- RushDB MCP server installed and running (see [quickstart](../mcp-server/quickstart.mdx))
...
This returns the full SearchQuery schema the query must conform to, including `where`, `aggregate`, `groupBy`, `orderBy`, `limit`, `skip`, and all filter operators (`$in`, `$gte`, `$contains`, `$exists`, `$relation`, `$alias`, etc.).
...
- [MCP tools reference](../mcp-server/tools.mdx) — full tool list with input schemas
- [Agent-Safe Query Planning with Ontology First](./agent-safe-query-planning.mdx) — building repeatable agent patterns with failure recovery
- [Hybrid Retrieval: Structured Filters Plus Semantic Search](./hybrid-retrieval.mdx) — combining semantic search with graph traversal
