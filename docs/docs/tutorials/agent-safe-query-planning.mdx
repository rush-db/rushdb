...
async function queryWithFallback(label: string, filter: Record<string, unknown>) {
  ...
  if (result.data.length === 0) {
    ...
    for (const key of Object.keys(filter)) {
      const narrower = Object.fromEntries(
        Object.entries(filter).filter(([k]) => k !== key)
      )
...
- [Discovery Queries](./discovery-queries.mdx) — interactive schema exploration in code
- [MCP Quickstart for Real Operators](./mcp-operator-quickstart.mdx) — the same loop via MCP tools
- [Building Team Memory](./building-team-memory.mdx) — a real knowledge base for agents to query
