...
4. Run an ANN search (fast, single label)
5. Run an ENN search with a prefilter (exact, with `where`)
...
## Step 4: ANN search (fast, single label)

Single label + no `where` = ANN mode: queries the global vector index directly.
...
## Step 5: ENN search with prefilter

Adding a `where` clause switches to exact nearest-neighbour mode: the graph is pre-filtered first, then cosine similarity is computed on the narrowed set.
...
- **Multiple labels**: Pass 2+ labels in `labels` — this also activates ENN mode and searches across all specified labels.
