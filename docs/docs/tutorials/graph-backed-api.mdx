...
## Pattern 3: Aggregate endpoints

Expose KPI endpoints that return counts, sums, and distributions — not raw records.
...
5. **Aggregate responses** return structured objects, not raw RushDB result arrays
...
- [Hybrid Retrieval: Structured Filters Plus Semantic Search](./hybrid-retrieval.mdx) — combining filter and vector search in one handler
- [Semantic Search for Multi-Tenant Products](./semantic-search-multitenant.mdx) — tenant isolation at the storage layer
- [Query Optimization and KU Efficiency](./query-optimization.mdx) — measuring and reducing compute cost
