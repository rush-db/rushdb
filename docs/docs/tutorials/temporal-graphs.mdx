...
## Step 6: Aggregate events by type over a time window
...
const upgrades = await db.records.find({
  ...
  aggregate: {
    eventCount: { fn: 'count', alias: '$record' },
    firstUpgrade: { fn: 'min', field: 'occurredAt', alias: '$record' },
    lastUpgrade: { fn: 'max', field: 'occurredAt', alias: '$record' }
  },
...
- [Audit Trails with Immutable Events and Derived State](./audit-trails.mdx) — extending this pattern for compliance use cases
- [RushDB as a Memory Layer](./memory-layer.mdx) — using the same FACT/EPISODE pattern for agent memory
- [SearchQuery Deep Dive](./searchquery-advanced-patterns.mdx) — date filter syntax and aggregation patterns
