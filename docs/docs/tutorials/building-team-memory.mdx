...
for item in related.data:
    print(f"[{item.__labels}] {item.data.get('title')} — score: {item.__score:.3f}")
```
...
def get_ticket_context(external_id: str) -> str:
    ...
    for t in similar.data:
        lines.append(f"- {t.data.get('externalId')}: {t.data.get('title')} (score: {t.__score:.2f})")
...
- **Webhooks from Linear/GitHub/PagerDuty** — auto-create TICKET and INCIDENT records on event (see [Event-Driven Ingestion](./event-driven-ingestion.mdx))
...
- [Agent-Safe Query Planning with Ontology First](./agent-safe-query-planning.mdx) — run agents over this memory graph safely
- [Episodic Memory for Multi-Step Agents](./episodic-memory.mdx) — session-scoped context alongside persistent team memory
- [Data Lineage](./data-lineage.mdx) — trace decisions back to source data
