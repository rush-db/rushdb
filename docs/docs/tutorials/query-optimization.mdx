...
| `fn: "collect"` in aggregates | Materializes all matched records into a list |
...
## Tip 4: Prefer `count` aggregates over `collect`

`fn: "collect"` materializes every matched record ID into a list before returning. `fn: "count"` just increments a counter. Use `collect` only when you need the actual IDs.
...
const collected = await db.records.find({
  ...
  aggregate: { ids: { fn: 'collect', alias: '$record' } }
})
...
```python
# ✅ Prefer count over collect unless IDs are needed
result = db.records.find({
    "labels": ["ORDER"],
    "where": {"status": "pending"},
    "aggregate": {"count": {"fn": "count", "alias": "$record"}}
...
| `fn: "collect"` replaced with `fn: "count"` where possible | `collect` materializes all matched IDs |
