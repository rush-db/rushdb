...
RushDB performs exact semantic search by applying label/optional `where` filtering before cosine ranking.
...
| where        | object           | no       | —       | Optional filter applied before cosine scoring |
