...
Return the complete RushDB SearchQuery specification as a focused reference document. Call this before building any `findRecords` query that involves dates, aggregation, `groupBy`, relationship traversal, or vector search.

Covers:
- All `where` operators: string, number, boolean, datetime component objects, vector, `$exists`, `$type`, logical grouping (`$and`/`$or`/`$not`/`$nor`/`$xor`)
